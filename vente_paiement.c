                  ...
                  ,ElementPaiement* listPaiement)
{
    ...
    nouvVente->listPaiement=listPaiement;
                                ...
                                ,ElementPaiement* listPaiement)
{
    ...
    nouv->vente=creatVente(codeVente ,date ,listCommande ,client,listPaiement);
    nouv->nxt=NULL;
    return nouv;
}
                                  ...
                                  ,ElementPaiement* listPaiement)
{
    ...
    nouv=CreatElementVente(codeVente ,date ,listCommande ,client,listPaiement);
...
void displayVente(vente* vente)
{
    ...
    display_liste_commande(vente->listCommande);}
     ...
     printf(" ------(    les paiements  concernant cette vente :   )----------_\n");
     if(test_list_paiement(vente->listPaiement))
        display_list_paiement(vente->listPaiement);
...
ElementVente*  ajouterVente(ElementCommande*  listVente)
{
    ...
    do
    {
     ...
     printf("------------  Les paiement :     ------  \n");
    ElementPaiement* listPaiement=NULL;

    do
    {
        printf(" * 1  :  ajouter un paiement  \n ");
        printf(" * 2  :  enregistrer          \n ");
        printf("----------------------------------\n");
        printf("  entrez votre choix : ");
        scanf("%d",&choix);
        switch(choix)
        {
        case 1: listPaiement=ajouterPaiement(listPaiement);break;
    ...
    listVente=ajouterElementVente(listVente,codeVente,date,listCommande,client,listPaiement);
}
...
{   int codeVente;
    ...
    for(tmp=listVente;tmp!=NULL;tmp=tmp->nxt)
    {
        if(tmp->vente->codeVente==code)
        {
            ...
            printf(" *  modifier la liste de paiements concernants cette vente : ");
            tmp->vente->listPaiement=mainPaiement(tmp->vente->listPaiement);
